# Ubuntu Noble (24.04 LTS) - Root.io Package Registry Configuration

This example demonstrates how to configure Ubuntu to use the Root.io APT package registry.
...
echo "deb [signed-by=/etc/apt/keyrings/rootio.gpg] https://root:YOUR_API_KEY@pkg.root.io/ubuntu/noble noble main" | \
...
```
https://root:${ROOTIO_API_KEY}@pkg.root.io/ubuntu/${RELEASE} ${RELEASE} main
```

- **Authentication**: HTTP Basic Auth with username `root` and your API key as password
- **Path**: `/ubuntu/${RELEASE}` (e.g., `/ubuntu/noble`)
- **Release**: Ubuntu codename (e.g., `focal`, `jammy`, `noble`)
- **Component**: `main`

### GPG Key

The Base64-encoded public key is the Root.io APT signing key used to verify package signatures. It's stored in `/etc/apt/keyrings/rootio.gpg` following Ubuntu best practices.
