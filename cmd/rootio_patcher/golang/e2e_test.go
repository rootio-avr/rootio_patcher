...
`)
	...
	assert.True(t, containsLine(got, "replace github.com/google/uuid v1.3.0 => github.com/google/uuid v1.3.0-rootio.1"))
...
replace github.com/google/uuid v1.3.0 => github.com/google/uuid v1.3.0-rootio.1
`)
	cmdRunner := &MockCommandRunner{}
	...
	assert.True(t, containsLine(got, "replace github.com/google/uuid v1.3.0 => github.com/google/uuid v1.3.0-rootio.2"))
	// revision 1 must no longer appear
	assert.False(t, strings.Contains(got, "v1.3.0-rootio.1"))
}

// TestGoLangApp_E2E_MultiplePatches verifies that when multiple modules are patched,
...
func TestGoLangApp_E2E_MultiplePatches(t *testing.T) {
	ctx := context.Background()
	logger := slog.New(slog.NewTextHandler(os.Stdout, &slog.HandlerOptions{Level: slog.LevelDebug}))
...
// TestGoLangApp_E2E_WithVendor verifies that go mod vendor is also called when a
// vendor/modules.txt file exists, after the same-path replace directive is written and
// go mod tidy runs — no `go get` step in between.
